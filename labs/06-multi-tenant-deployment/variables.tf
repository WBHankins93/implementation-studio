variable "cloud_provider" {
  description = "Cloud provider: kind (local), gcp, or aws"
  type        = string
  default     = "kind"
  validation {
...
variable "region" {
  description = "Cloud region (GCP region or AWS region, ignored for Kind)"
  type        = string
  default     = "us-central1" # Default to GCP region, user should change for AWS
}

variable "public_subnet_cidr" {
  description = "CIDR block for public subnet (GCP/AWS only)"
  type        = string
  default     = "10.0.1.0/24"
}

variable "private_subnet_cidr" {
  description = "CIDR block for private subnet (GCP/AWS only)"
  type        = string
  default     = "10.0.2.0/24"
}

variable "node_count" {
  description = "Number of nodes (per zone for GCP, desired count for AWS, ignored for Kind)"
  type        = number
  default     = 2
}

variable "min_node_count" {
  description = "Minimum number of nodes (GCP/AWS only)"
  type        = number
  default     = 1
}

variable "max_node_count" {
  description = "Maximum number of nodes (GCP/AWS only)"
  type        = number
  default     = 5
}

variable "resource_labels" {
  description = "Labels/tags to apply to cluster resources (GCP/AWS only)"
  type        = map(string)
  default = {
